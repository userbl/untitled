...
def saveToDB(data):
    ...
    global connection
    try:
        connection = psycopg2.connect(host='192.168.124.123',
                                      user='cat',
                                      password='cat',
                                      database='my-dev' )
        insert_count =0;
        for dd in data :
            with connection.cursor() as cursor:
                ...
                sql = 'SELECT count(1) cc FROM tech_article_hot WHERE title=%s and url=%s'
            ...
            if result[0] == 0:
                with connection.cursor() as cursor:
                    # Create a new record
                    sql = "INSERT INTO tech_article_hot (title, url,date_str,where_come_from) VALUES (%s, %s,%s,%s)"
...
def grab() :
        ...
        detail_count =0;
        urls=['https://toutiao.io/posts/hot/7','https://toutiao.io/posts/hot/30','https://toutiao.io/posts/hot/90']

        for a in urls:


            detail = get_news_page(a);
